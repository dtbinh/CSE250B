...
Repeat T times
	...
	for i = 1 to n:
		if (x_i, y_i) is misclassified by w_l:
			...
			c(l) = c(l) + 1
			if l > L & c(l) is larger than the min[c(1...l-1)]
				// a MIN-Heap may be used here for efficiency
				delete the w with smallest c from storage
				store w(l)
return all w that is in storage
\end{minted}
